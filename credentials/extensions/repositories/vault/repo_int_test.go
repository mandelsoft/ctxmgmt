...
var _ = Describe("vault config", func() {
	...
	Context("authentication to vault and reading secrets", func() {
		...
		It("authenticate with token and retrieve credentials", func() {
			...
			consumerId := Must(vault.GetConsumerId(vaultClient.Configuration().Address,
				"", "secret", VAULT_PATH_REPO1))
			creds := credentials.NewCredentials(utils.Properties{
				vault.ATTR_AUTHMETH: vault.AUTH_TOKEN,
				vault.ATTR_TOKEN:    VAULT_ROOT_TOKEN,
			})
...
`, VAULT_PATH_REPO2, VAULT_PATH_REPO2)}))
			...
			consumerId := Must(vault.GetConsumerId(VAULT_HTTP_URL, "", "secret", VAULT_PATH_REPO2))
			data := map[string]any{
				vault.ATTR_AUTHMETH: vault.AUTH_APPROLE,
				vault.ATTR_ROLEID:   roleid,
				vault.ATTR_SECRETID: secretid,
			}
			_ = Must(vaultClient.Secrets.KvV2Write(ctx, VAULT_PATH_REPO1+"/"+VAULT_SECRET,
				schema.KvV2WriteRequest{Data: data},
				vault.WithMountPath("secret"),
...
func SetUpVaultAccess(ctx context.Context, credctx credentials.Context, client *vault.Client, policy string) {
	...
	consumerId := Must(vault.GetConsumerId(client.Configuration().Address, "", "secret", VAULT_PATH_REPO1))
	creds := credentials.NewCredentials(utils.Properties{
		vault.ATTR_AUTHMETH: vault.AUTH_APPROLE,
		vault.ATTR_ROLEID:   roleid,
		vault.ATTR_SECRETID: secretid,
