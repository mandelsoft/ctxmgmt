...
func (c *ConfigFragment) GetLogConfig(fss ...vfs.FileSystem) (*config.Config, error) {
	...
	if c.LogConfig != "" {
		...
		if strings.HasPrefix(c.LogConfig, "@") {
			data, err = optionutils.ReadFile(c.LogConfig[1:], general.OptionalDefaulted[vfs.FileSystem](osfs.OsFs, fss...))
			if err != nil {
...
func (c *ConfigFragment) Evaluate(ctx datacontext.Context, logctx logging.Context, main bool) (*EvaluatedOptions, error) {
	...
	fs := vfsattr.Get(ctx)
	if main && c.LogFileName != "" && logging2.GlobalLogFileOverride == "" {
		if opts.LogFile == nil {
			opts.LogFile, err = logging2.LogFileFor(c.LogFileName, fs)
			if err != nil {
				return nil, errors.Wrapf(err, "cannot open log file %q", opts.LogFile)
			}
		}
		logging2.ConfigureLogrusFor(logctx, !c.Json, opts.LogFile)
		if logctx == logging.DefaultContext() {
			logging2.GlobalLogFile = opts.LogFile
		}
	} else {
